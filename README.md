...
2. Далее в окне `настрока соеденения` заполняем все необходимые поля(Прошу обратить внимание на поле в номером порта в данноим случае мы используем **ПОРТ** `5434` так как наружу мы прокинули именно этот порт и в поле **ХОСТ** мы указываем именно `localhost`)
