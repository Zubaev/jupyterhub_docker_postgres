...
- [Домашняя работа №1](#домашняя-работа-1)
  ...
  - [Регистрации пользователей jupyterhub](#2-Регистрации-пользователей-jupyterhub)
  - [Postgres](#3-Postgres)
...
- [Домашняя работа №2](#домашняя-работа-2)
  ...
  - [3. Дашборды](#3-Дашборды)
    - [3.1 Дашборд размера тетрадок Jupyterhub.](#31-Дашборд-размера-тетрадок-Jupyterhub)
    - [3.2 Дашборд Активности пользователей.](#32-Дашборд-активности-пользователей)
    - [3.3 Дашборд размера таблиц в PostgreSQL.](#32-Дашборд-размера-таблиц-в-PostgreSQL)
...
Список зарегистрированных пользователей можно провенрить на панели администратора по адресу `http://localhost:8000/hub/admin#/`
...
***
## 3. Postgres
Для Проверки работоспособности `Postgres` я буду использовать приложение `DBeaver` 

1. Устанавливаем новое соеденение в выпавшем списке необходимо выбрать `Postgres`
2. Далее в окне `настрока соеденения` заполняем все необходимые поля(Прошу обратить внимание на поле с номером порта в данноим случае мы используем **ПОРТ** `5434` так как наружу мы прокинули именно этот порт и в поле **ХОСТ** мы указываем именно `localhost`)
3. Жмем `Тест соеденения`
4. Если все правильно мы увидим окошко потдверждающее это

![2025-03-05_23-38-03](https://github.com/user-attachments/assets/4f35bea3-42a1-450f-9983-4f49824f85df)
...
2. Далее прописываем все необходимые настройки для соеденинения(В `DBeaver` использовался  **ПОРТ** `5434`, а теперь `5432` и вместо `localhost` мы используем имя контейнера `db_postgres` потому что оба контейнера у нас находятся в одной подсети `jupyter-network`)
3. Создаем запрос на создание таблицы и если все сделали правильно внизу вы увидите вывод строк которые добавились в таблицу
